...
class ProfilPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final gender = user?.jenisKelamin ?? '';
    final training = user?.training?.title ?? '';
    final batch = user?.batch?.batchKe ?? '';

    // Status check logic
    final now = DateTime.now();
    String status = 'Tidak Aktif';
    final endDate = user?.batch?.endDate;
    ...
    return SafeArea(
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            _buildHeader(name, email, initials, status, user?.profilePhoto),
            _buildStatCard(),
            Padding(
